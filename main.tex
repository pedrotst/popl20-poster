...
The main challenge is that, in Coq, kinds, types and terms are syntatically the same thing. In Cedille we have to clearly differentiate between them.

Our main approach is to access the metacoq AST and translate it to the cedille
counterpart by using the monadic constructs provided by ext-lib\textsuperscript{3}. The main challange
is to come up with a sound approach to decide wether a gallina term lives in a
cedille term, type or kind.
...
\begin{tabular}[t]{p{.6\linewidth}r}
    ...
    \textit{If you want to see the proof written with tactics, take a look at the project github\textsuperscript{3}}.
...
\end{tabular}
 ...
 Bohm-out algorithm\textsuperscript{4}. If the terms being compared are not $\beta\eta$-equivalent, $\delta$
